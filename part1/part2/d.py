...
if __name__ == '__main__':
    ...
    # writing the average to a new parquet file
    df = pd.DataFrame(list(avg.items()), columns=['time', 'average'])
    df.to_parquet('output_avg.parquet', index=False)
    
"""
פארקט שומר את הנתונים בעמודות בזיכרון, לעומת אקסל ששומר את הנתונים שלו  
טקסט רגיל. לכן בפארקט אני יכולה לגשת לעמודה ספציפית בלי צורך לקרוא את כל הטקסט 
מה שמפחית בצורה משמעותית את זמן הריצה.

 פארקט לא מוגבל מבחינת זיכרון ותופס פחות מקום(הוא דוחס את הנתונים) 

פארקט שומר על כל עמודה מהו סוג הנתונים וכך הוא שומר על עקביות הנתונים,
 לעומת זאת אקסל לא שומר טיפוסי נתונים הוא קורא את הקובץ ולפי זה מפרש את סוג הנתון
   מה שיכול לגרום לשגיאות וחוסר עקביות.
"""
